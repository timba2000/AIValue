...
**Role-Based Access Control (Dec 2025):** Three-tier permission system with Reader, Editor, and Admin roles. Readers can view all data but cannot modify anything. Editors can create, update, and delete companies, business units, processes, pain points, and solutions. Admins have full access including user management, role changes, and system configuration. Role is stored in users table with default 'reader'. Backend middleware (`isEditorOrAdmin`) protects all mutation routes (POST/PUT/DELETE). Frontend `useAuth` hook exposes `role`, `isEditor`, `canEdit` for UI gating. Admin user management page allows role changes via dropdown with safeguard against demoting the last admin.
