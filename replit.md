...
**Audit Logging (Dec 2025):** Comprehensive audit trail for all data changes. Tracks create, update, and delete operations on pain points and solutions with before/after values, user identification, timestamps, and IP addresses. Admin UI at /admin/audit shows filterable, paginated logs with summary statistics (last 24h activity, action counts). Detail modal displays full change history including field-level diffs. Entity names are stored for readability even after deletion. Backend service (backend/src/services/auditLog.ts) provides reusable logging utilities. Inline edits in the Pain Points Overview table now preserve row position after saves (no auto-resort until user clicks column header) and include all fields to prevent data loss.
